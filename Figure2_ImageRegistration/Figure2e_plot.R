...
for(typei in c(1,2)){
    ...
    saving_path = sprintf("./Figure2e_%s.eps", type);
    ...
    BnB_mean = res$rt.BnB.mean[1:l, 1];
    BnB_std = res$rt.BnB.std[1:l, 1];

    APM_mean = res$rt.APM.mean[1:l, 1];
    APM_std = res$rt.APM.std[1:l, 1];
    ...
    p = ggplot(data=dat, mapping=aes(x=missing_ratios, y=means,
      ...
      labs(x="Outlier Ratio",y="Running Time (Sec)", title=TeX(titlename)) +
