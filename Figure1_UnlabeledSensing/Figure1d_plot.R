...
point_size = 7;
    ...
    AltMin_mean = res$es.AltMin.mean[start:end, 1];
    AltMin_std = res$es.AltMin.std[start:end, 1];

    GRplus_mean = res$es.GR.mean[start:end,1];
    GRplus_std = res$es.GR.std[start:end,1];

    means = c(BnB_mean, AltMin_mean, GRplus_mean);
    stds = c(BnB_std, AltMin_std, GRplus_std);

    noise_ratios = res$noise.ratios[1,start:end];
    noise_ratios = rep(c(noise_ratios), 3);
    ...
    p = ggplot(data=dat, mapping=aes(x=noise_ratios, y=means,
      ...
      expand_limits(y=0.001,x=0.101) +

      labs(x="Noise Level",y="Estimation Error", title=TeX(titlename)) +

      theme(axis.text=element_text(face="plain", size=28, color='gray10'),
            legend.text=element_text(face="plain",size=28, family="mono"),
            # title=element_blank(),
